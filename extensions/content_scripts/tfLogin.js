class TFAuth 
{
    // constructor function for TFAuth class
	constructor(usr,pwd) 
	{ 
        ...
        // Return the TFAuth object
        return this;
	}
}


        // Function to prompt the user for their account ID and password, then create a new TFAuth object with those credentials
        const logon=(p = true,usr,pwd)=>
        {
            if(p) { var usr = prompt("Account ID","",""); var pwd = prompt("Password","paradyne",""); }
            
            var auth = new TFAuth(usr,pwd);
            return auth;
        }


		var dataSetFound = (document.querySelector("[data-brand]") !== null);
			//chrome.runtime.sendMessage({type: 'datasetFound',exists: exampleElementExists});

		if(dataSetFound) { 	logon(); }
